/**
 * Antigua request-to-book Worker (Classic Beach Day only).
 *
 ...
 */
import { antiguaBookingCore } from "../../../shared/destinations/antigua";
...
async function handlePreviewRequest(request: Request, env: Env): Promise<Response> {
    ...
    return jsonResponse(
      {
        ...
        message:
          "Live Stripe is not enabled for this Antigua Worker. Charge-then-refund still requires an explicit production configuration. No payment was taken.",
      },
      503,
    );
  }
  ...
  const product = findAntiguaBookingProduct(body.productId);
  ...
  const reference = createBookingReference(antiguaBookingCore);
  ...
  const destinationBrand = destinationBrandFromCore(antiguaBookingCore);
