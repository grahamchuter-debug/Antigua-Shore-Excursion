/**
 * Antigua request-to-book form (Phase 15D).
 * Server-side pricing is authoritative — client total is for review only.
 * Guest model: paying guests (ages 3+) + free infants (0–2). No child discount.
 */
(function () {
  const form = document.getElementById("atg-booking-form");
  if (!form || !window.ATG_COMMERCIAL) return;

  const cfg = window.ATG_COMMERCIAL;
