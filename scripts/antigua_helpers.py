"""HTML helpers for Antigua Shore Excursion site build."""
import json
...
def snapshot_default(**overrides: str) -> str:
    defaults = dict(
        ...
        best_for="Beach days, Nelson's Dockyard, kayak & snorkel, island culture",
        ...
        popular="Beach days, history tours, kayak snorkel, scenic drives",
    )
...
def internal_links(extra: list[tuple[str, str]] | None = None) -> str:
    base = [
        ("antigua-port-guide.html", "Port Guide"),
        ("best-antigua-shore-excursions.html", "Best Excursions"),
        ("one-day-in-antigua-from-a-cruise-ship.html", "One Day Itinerary"),
        ("nelsons-dockyard-from-antigua-cruise-port.html", "Nelson's Dockyard"),
        ("shirley-heights-antigua-guide.html", "Shirley Heights"),
        ("best-beaches-in-antigua-for-cruise-passengers.html", "Best Beaches"),
    ]
    if extra:
        base.extend(extra)
    parts = []
...
def content_tour_page(
    ...
    highlights_subtitle: str = "What to expect on this Antigua shore excursion.",
...
def comparison_section() -> str:
    rows = [
        ("The Antiguan Experience", "4 hrs", "Culture, pineapple farm &amp; beach", "Moderate", "the-antiguan-experience.html"),
        ("Classic Beach Day", "4 hrs", "White sand beach with lunch", "Moderate", "classic-beach-day.html"),
        ("Kayak, Snorkel &amp; Beach", "4 hrs", "Mangroves, Cades Reef &amp; beach", "Moderate", "antigua-kayak-snorkel-and-beach.html"),
        ("History &amp; Culture Tour", "4 hrs", "Nelson's Dockyard &amp; Shirley Heights", "Easy", "antigua-exclusive-history-and-culture-tour.html"),
        ("Panoramic &amp; Beach Break", "4.5 hrs", "Island viewpoints &amp; beach time", "Easy", "panoramic-antigua-and-beach-break.html"),
        ("Half Day Kayak &amp; Snorkel", "4.5 hrs", "Mangrove lagoon &amp; Cades Reef", "Moderate", "antigua-half-day-kayak-and-snorkel.html"),
    ]
    body = ""
  ...
  <h2 class="text-3xl sm:text-4xl font-display font-bold text-gray-900 text-center mb-4">Which Antigua Excursion Is Right for Me?</h2>
  <p class="text-center text-gray-600 text-sm max-w-2xl mx-auto mb-10">Match your St John's port day to beach escapes, Nelson's Dockyard history, kayak snorkel adventures and scenic island drives — all timed for typical 6–10 hour cruise calls in Antigua and Barbuda.</p>
        ...
        "@graph": [
            {
                ...
                "name": SITE,
                "url": f"{DOMAIN}/",
                "description": "Planning guide for Antigua cruise shore excursions from St John's, Antigua and Barbuda",
            },
            {
                "@type": "LocalBusiness",
                "name": SITE,
                "url": f"{DOMAIN}/",
                "description": "Cruise passenger planning guide for St John's Antigua shore excursions in Antigua and Barbuda",
                "address": {
                    "@type": "PostalAddress",
                    "addressLocality": "St John's",
                    "addressRegion": "Saint John Parish",
                    "addressCountry": "AG",
                },
                "areaServed": {
                    "@type": "City",
